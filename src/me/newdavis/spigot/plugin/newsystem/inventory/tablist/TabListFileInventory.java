...
public class TabListFileInventory {
    ...
    public void openInventoryPage(Player p, int inventoryPage) {
        ...
        if(!(inventoryPage > maxPages)) {
            ...
            if (inventoryPage > 0) {
                if (inventoryPage == 1) {
                    for (String tablist : TabListFile.getConfigurationSection("TabList")) {
                        if (inventory.getItem(43) == null || inventory.getItem(43).getType() == Material.AIR) {
                            if(tablist.equalsIgnoreCase("Enabled") || tablist.equalsIgnoreCase("RoleNotFound")) {
                                Object value = TabListFile.yaml.get("TabList." + tablist);
                    ...
                    for (int i = (inventoryPage-1) * 28; i < (inventoryPage-1) * 28 + 29; i++) {
                        if (inventory.getItem(43) == null || inventory.getItem(43).getType() == Material.AIR) {
                            if(tablists.size() > (i)) {
                                if(tablists.get(i).equalsIgnoreCase("Enabled") || tablists.get(i).equalsIgnoreCase("RoleNotFound")) {
                                    Object value = TabListFile.yaml.get("TabList." + tablists.get(i));
                                    if (!String.valueOf(value).contains("MemorySection")) {
                                        List<String> lore = new ArrayList<>();
                                        lore.add("§7Derzeitiger Wert: §7");
                                        lore.add("§7" + String.valueOf(value).replace("[", "").replace("]", "").replace(", ", "§8'§7 §8'§7"));
                                        ItemStack item = new ItemBuilder(Material.PAPER)
                                                .setName("§7" + tablists.get(i))
                                                .setLore(lore)
                                                .build();
                                        inventory.addItem(item);
                                    }
                                    ...
                                    ItemStack item = new ItemBuilder(Material.BOOK)
                                            .setName("§7TabList: " + tablists.get(i))
                                            .build();
                                    inventory.addItem(item);
                                }
                            }
