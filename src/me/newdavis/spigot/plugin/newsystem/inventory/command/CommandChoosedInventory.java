...
public class CommandChoosedInventory {
    ...
    public void openInventoryPage(Player p, String inventoryCommand, int inventoryPage) {
        if(CommandFile.isPathSet("Command." + inventoryCommand)) {
            Inventory inventory = Bukkit.createInventory(null, 9 * rows, TITLE);

            List<String> paths = getPaths(inventoryCommand);
                ...
                if (inventoryPage == 1) {
                    for (String path : paths) {
                        if (inventory.getItem(43) == null || inventory.getItem(43).getType() == Material.AIR) {
                            Object value = CommandFile.yaml.get("Command." + inventoryCommand + "." + path);
                    ...
                    for (int i = (inventoryPage - 1) * 28; i < (inventoryPage - 1) * 28 + 29; i++) {
                        if (inventory.getItem(43) == null || inventory.getItem(43).getType() == Material.AIR) {
                            if (paths.size() > i) {
                                Object value = CommandFile.yaml.get("Command." + inventoryCommand + "." + paths.get(i));
                                if (!String.valueOf(value).contains("MemorySection")) {
                                    List<String> lore = new ArrayList<>();
                                    lore.add("§7Derzeitiger Wert: §7");
                                    lore.add("§7" + String.valueOf(value).replace("[", "").replace("]", "").replace(", ", "§8'§7 §8'§7"));
    ...
    private List<String> getPaths(String command) {
        if(CommandFile.isPathSet("Command." + command)) {
            Set<String> keys = CommandFile.yaml.getConfigurationSection("Command." + command).getKeys(true);
