...
public class SettingChoosedInventory {
    ...
    public void openInventoryPage(Player p, String inventorySetting, int inventoryPage) {
        if(inventorySetting.equalsIgnoreCase("Time") || inventorySetting.equalsIgnoreCase("Date") || inventorySetting.equalsIgnoreCase("MySQL")) {
            Inventory inventory = Bukkit.createInventory(null, 9 * rows, TITLE);

            List<String> paths = getPaths(inventorySetting);
                ...
                if (inventoryPage == 1) {
                    for (String path : paths) {
                        if (inventory.getItem(43) == null || inventory.getItem(43).getType() == Material.AIR) {
                            Object value = SettingsFile.yaml.get(inventorySetting + "." + path);
                    ...
                    for (int i = (inventoryPage - 1) * 28; i < (inventoryPage - 1) * 28 + 29; i++) {
                        if (inventory.getItem(43) == null || inventory.getItem(43).getType() == Material.AIR) {
                            if (paths.size() > i) {
                                Object value = SettingsFile.yaml.get(inventorySetting + "." + paths.get(i));
                                if (!String.valueOf(value).contains("MemorySection")) {
                                    List<String> lore = new ArrayList<>();
                                    lore.add("§7Derzeitiger Wert: §7");
                                    lore.add("§7" + String.valueOf(value).replace("[", "").replace("]", "").replace(", ", "§8'§7 §8'§7"));
    ...
    private List<String> getPaths(String setting) {
        if(SettingsFile.isPathSet(setting)) {
            Set<String> keys = SettingsFile.yaml.getConfigurationSection(setting).getKeys(true);
