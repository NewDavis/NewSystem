...
public class SettingsFileInventory {
                ...
                if (inventoryPage == 1) {
                    for (String setting : settings) {
                        if (inventory.getItem(43) == null || inventory.getItem(43).getType() == Material.AIR) {
                            if(!(setting.equalsIgnoreCase("Time") || setting.equalsIgnoreCase("Date") || setting.equalsIgnoreCase("MySQL"))) {
                                Object value = SettingsFile.yaml.get(setting);
                    ...
                    for (int i = (inventoryPage-1) * 28; i < (inventoryPage-1) * 28 + 29; i++) {
                        if (inventory.getItem(43) == null || inventory.getItem(43).getType() == Material.AIR) {
                            if(settings.size() > (i)) {
                                if(!(settings.get(i).equalsIgnoreCase("Time") || settings.get(i).equalsIgnoreCase("Date") || settings.get(i).equalsIgnoreCase("MySQL"))) {
                                    Object value = TabListFile.yaml.get(settings.get(i));
                                    if (!String.valueOf(value).contains("MemorySection")) {
                                        List<String> lore = new ArrayList<>();
                                        lore.add("§7Derzeitiger Wert: §7");
                                        lore.add("§7" + String.valueOf(value).replace("[", "").replace("]", "").replace(", ", "§8'§7 §8'§7"));
                                        ItemStack item = new ItemBuilder(Material.PAPER)
                                                .setName("§7" + settings.get(i))
                                                .setLore(lore)
                                                .build();
                                        inventory.addItem(item);
                                    }
                                    ...
                                    ItemStack item = new ItemBuilder(Material.BOOK)
                                            .setName("§7Setting: " + settings.get(i))
                                            .build();
                                    inventory.addItem(item);
                                }
                            }
