...
public class KitChoosedInventory {
    ...
    public void openInventoryPage(Player p, String inventoryKit, int inventoryPage) {
        if (KitFile.isPathSet("Kit." + inventoryKit)) {
            Inventory inventory = Bukkit.createInventory(null, 9 * rows, TITLE);

            List<String> paths = getPaths(inventoryKit);
                ...
                if (inventoryPage == 1) {
                    for (String path : paths) {
                        if (inventory.getItem(43) == null || inventory.getItem(43).getType() == Material.AIR) {
                            Object value = (KitFile.yaml.get("Kit." + inventoryKit + "." + path) == null ? KitFile.yaml.get("Kit." + inventoryKit) : KitFile.yaml.get("Kit." + inventoryKit + "." + path));
                    ...
                    for (int i = (inventoryPage - 1) * 28; i < (inventoryPage - 1) * 28 + 29; i++) {
                        if (inventory.getItem(43) == null || inventory.getItem(43).getType() == Material.AIR) {
                            if (paths.size() > i) {
                                Object value = (KitFile.yaml.get("Kit." + inventoryKit + "." + paths.get(i)) == null ? KitFile.yaml.get("Kit." + inventoryKit) : KitFile.yaml.get("Kit." + inventoryKit + "." + paths.get(i)));
                                if (!String.valueOf(value).contains("MemorySection")) {
                                    List<String> lore = new ArrayList<>();
                                    lore.add("§7Derzeitiger Wert: §7");
                                    lore.add("§7" + String.valueOf(value).replace("[", "").replace("]", "").replace(", ", "§8'§7 §8'§7"));
            ...
            kit.put(p, inventoryKit);
        } else {
            new KitFileInventory().openInventoryPage(p, 1);
        }
    }

    private List<String> getPaths(String kit) {
        if(KitFile.isPathSet("Kit." + kit)) {
            Set<String> keys = KitFile.yaml.getConfigurationSection("Kit." + kit).getKeys(true);
