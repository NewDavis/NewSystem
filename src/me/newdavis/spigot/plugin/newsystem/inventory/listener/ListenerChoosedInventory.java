...
public class ListenerChoosedInventory {

    public static HashMap<Player, Integer> page = new HashMap<>();
    public static HashMap<Player, String> listener = new HashMap<>();

    public final String TITLE = "§8» §fEvent ";
    ...
    public void openInventoryPage(Player p, String inventoryEvent, int inventoryPage) {
        if (ListenerFile.isPathSet("Listener." + inventoryEvent)) {
            Inventory inventory = Bukkit.createInventory(null, 9 * rows, TITLE);

            List<String> paths = getPaths(inventoryEvent);
                ...
                if (inventoryPage == 1) {
                    for (String path : paths) {
                        if (inventory.getItem(43) == null || inventory.getItem(43).getType() == Material.AIR) {
                            Object value = ListenerFile.yaml.get("Listener." + inventoryEvent + "." + path);
                    ...
                    for (int i = (inventoryPage - 1) * 28; i < (inventoryPage - 1) * 28 + 29; i++) {
                        if (inventory.getItem(43) == null || inventory.getItem(43).getType() == Material.AIR) {
                            if (paths.size() > i) {
                                Object value = ListenerFile.yaml.get("Listener." + inventoryEvent + "." + paths.get(i));
                                if (!String.valueOf(value).contains("MemorySection")) {
                                    List<String> lore = new ArrayList<>();
                                    lore.add("§7Derzeitiger Wert: §7");
                                    lore.add("§7" + String.valueOf(value).replace("[", "").replace("]", "").replace(", ", "§8'§7 §8'§7"));
            ...
            listener.put(p, inventoryEvent);
        } else {
            new ListenerFileInventory().openInventoryPage(p, 1);
        }
    }

    private List<String> getPaths(String listener) {
        if(ListenerFile.isPathSet("Listener." + listener)) {
            Set<String> keys = ListenerFile.yaml.getConfigurationSection("Listener." + listener).getKeys(true);
